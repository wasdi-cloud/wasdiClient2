...
export class EditStyleDialogComponent implements OnInit {
  ...
  updateStyle() {
    ...
    this.m_oStyleService.updateStyleParameters(this.m_oStyle.styleId, this.m_oEditStyleForm.sDescription, this.m_oEditStyleForm.bIsPublic).subscribe({
      next: oResponse => {
        this.m_oNotificationDisplayService.openSnackBar("STYLE UPDATED", "Close", "right", "bottom");
        this.onDismiss(); 
      },
      error: oError => {
        this.m_oNotificationDisplayService.openAlertDialog( "ERROR IN UPDATING THIS STYLE");
      }
    })
  }
  ...
  updateStyleXML(sStyleId: string, sStyleXml: string) {
    ...
    if (FadeoutUtils.utilsIsStrNullOrEmpty(sStyleId) === false) {
      this.m_oStyleService.postStyleXml(sStyleId, oBody).subscribe({
        next: oResponse => {
          this.m_oNotificationDisplayService.openSnackBar("STYLE XML UPDATED", "Close", "right", "bottom");
          this.onDismiss();
        },
        error: oError => {
          this.m_oNotificationDisplayService.openAlertDialog( "ERROR IN UPDATING STYLE XML");
        }
      })
    }
  }
