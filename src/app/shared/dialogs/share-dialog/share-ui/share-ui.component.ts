...
export class ShareUiComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (
      FadeoutUtils.utilsIsObjectNullOrUndefined(this.resource.resourceId) ===
      false
    ) {
    ...
    if (this.resourceType && this.resource) {
      if (this.resourceType === 'workspace') {
        this.m_oWorkspaceService
          .getUsersBySharedWorkspace(this.resource.workspaceId)
          .subscribe((oResponse) => {
            if (oResponse) {
              this.m_aoSharedUsers = oResponse;
              this.m_bLoadingUsers = false;
              this.m_aoSharedUsers.length > 0
      ...
      if (this.resourceType === 'style') {
        this.m_oStyleService
          .getUsersBySharedStyle(this.resource.styleId)
          .subscribe((oResponse) => {
            if (oResponse) {
              this.m_aoSharedUsers = oResponse;
              this.m_bLoadingUsers = false;
      ...
      if (this.resourceType === 'PROCESSORPARAMETERSTEMPLATE') {
        this.m_oProcessorParametersTemplateService
          .getEnabledUsers(this.resource.templateId)
          .subscribe({
            next: (oResponse) => {
              if (
                FadeoutUtils.utilsIsObjectNullOrUndefined(oResponse) === false
              ) {
                this.m_aoSharedUsers = oResponse;
                this.m_bLoadingUsers = false;
                this.m_aoSharedUsers.length > 0
      ...
      if (this.resourceType === 'workflow') {
        this.m_oWorkflowService
          .getUsersBySharedWorkflow(this.resource.workflowId)
          .subscribe({
            next: (oResponse) => {
              if (oResponse) {
                this.m_aoSharedUsers = oResponse;
      ...
      if (this.resourceType === 'processor') {
        this.m_oProcessorService
          .getUsersBySharedProcessor(this.resource.processorId)
          .subscribe((oResponse) => {
            if (
              FadeoutUtils.utilsIsObjectNullOrUndefined(oResponse) === false
            ) {
      ...
      if (this.resourceType === 'organization') {
        this.m_oOrganizationsService
          .getUsersBySharedOrganization(this.resource.organizationId)
          .subscribe({
            next: (oResponse) => {
              this.m_aoSharedUsers = oResponse;
              this.m_bLoadingUsers = false;
      ...
      if (this.resourceType === 'subscription') {
        this.m_oSubscriptionService
          .getUsersBySharedSubscription(this.resource.subscriptionId)
          .subscribe({
            next: (oResponse) => {
              this.m_aoSharedUsers = oResponse;
              this.m_bLoadingUsers = false;
  ...
  onAddEnabledUser() {
    if (FadeoutUtils.utilsIsStrNullOrEmpty(this.m_sUserIdSearch)) {
      this.m_oNotificationDisplayService.openAlertDialog(
        this.m_oTranslate.instant('DIALOG_SHARE_ENTER_USER'),
        '',
        'alert'
      );
    }
    ...
    if (
      this.resourceType &&
      this.resource &&
      this.m_sUserIdSearch.length !== 0
    ) {
      if (this.resourceType === 'workspace') {
        this.m_oWorkspaceService
          .putShareWorkspace(
            this.resource.workspaceId,
            this.m_sUserIdSearch,
            this.m_sPermission
          )
          .subscribe({
      ...
      if (this.resourceType === 'PROCESSORPARAMETERSTEMPLATE') {
        this.m_oProcessorParametersTemplateService
          .shareProcessorParameterTemplate(
            this.resource.templateId,
            this.m_sUserIdSearch,
            this.m_sPermission
          )
          .subscribe({
      ...
      if (this.resourceType === 'subscription') {
        this.m_oSubscriptionService
          .addSubscriptionSharing(
            this.resource.subscriptionId,
            this.m_sUserIdSearch,
            this.m_sPermission
          )
          .subscribe({
            next: (oResponse) => {
              if (
                FadeoutUtils.utilsIsObjectNullOrUndefined(oResponse) ===
                  false &&
                oResponse.message === 'Done'
              ) {
                let sMessage = this.m_oTranslate.instant('KEY_PHRASES.SUCCESS');
                this.m_oNotificationDisplayService.openSnackBar(
                  sMessage,
            ...
            error: (oError) => {
              let sErrorMsg = this.m_oTranslate.instant(
                'DIALOG_SHARE_SUBSCRIPTION_ERROR'
              );
              this.m_oNotificationDisplayService.openAlertDialog(
                sErrorMsg,
                '',
      ...
      if (this.resourceType === 'mission') {
        this.m_oAdminDashboardService
          .addResourcePermission(
            'mission',
            this.resource.missionIndexValue,
            this.m_sUserIdSearch,
            this.m_sPermission
          )
          .subscribe({
            next: (oResponse) => {
              this.m_oNotificationDisplayService.openSnackBar(
                'User permissions added to mission',
                'Success',
                'success-snackbar'
              );
              this.getEnabledUsers();
            ...
            error: (oError) => {
              this.m_oNotificationDisplayService.openAlertDialog(
                'Could not add user permissions to mission',
                'danger'
              );
            },
          });
  ...
  onRemoveEnabledUser(sUserId: string) {
    ...
    if (this.resourceType && this.resource) {
      if (this.resourceType === 'workspace') {
        this.m_oWorkspaceService
          .deleteUserSharedWorkspace(this.resource.workspaceId, sUserId)
          .subscribe({
            next: (oResponse) => {
              if (
                oResponse.intValue === 200 &&
                oResponse.stringValue === 'Done'
              ) {
                this.getEnabledUsers();
              }
            },
      ...
      if (this.resourceType === 'style') {
        this.m_oStyleService
          .removeStyleSharing(this.resource.styleId, sUserId)
          .subscribe({
            next: (oResponse) => {
              if (
                oResponse.intValue === 200 &&
      ...
      if (this.resourceType === 'workflow') {
        this.m_oWorkflowService
          .removeWorkflowSharing(this.resource.workflowId, sUserId)
          .subscribe({
            next: (oResponse) => {
              if (oResponse.stringValue === 'Unauthorized') {
                this.m_oNotificationDisplayService.openAlertDialog(
                  this.m_oTranslate.instant('DIALOG_SHARE_PERMISSION_ERROR'),
                  '',
                  'alert'
                );
              } else {
      ...
      if (this.resourceType === 'processor') {
        this.m_oProcessorService
          .deleteUserSharedProcessor(this.resource.processorId, sUserId)
          .subscribe({
            next: (oResponse) => {
              this.getEnabledUsers();
            },
    ...
    if (this.resourceType === 'organization') {
      this.m_oOrganizationsService
        .removeOrganizationSharing(this.resource.organizationId, sUserId)
        .subscribe({
          next: (oResponse) => {
            let sMessage = this.m_oTranslate.instant('KEY_PHRASES.SUCCESS');
            this.m_oNotificationDisplayService.openSnackBar(
    ...
    if (this.resourceType === 'subscription') {
      this.m_oSubscriptionService
        .removeSubscriptionSharing(this.resource.subscriptionId, sUserId)
        .subscribe({
          next: (oResponse) => {
            let sMessage = this.m_oTranslate.instant('KEY_PHRASES.SUCCESS');
            this.m_oNotificationDisplayService.openSnackBar(
